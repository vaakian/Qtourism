...
const PackageItem = ({ package: p }) => {
  ...
  const takeOrder = useMutation(() => {
    const { id } = p;
    return userService.order({ packageId: id }).then(({ data }) => {
    ...
    <div className="package m-2">
      <Card
        ...
        actions={[
          ...
          <Popover
            content={<MerchantInfo merchant={p.merchant} />}
            title="商家信息"
          >
          ...
          <Button
            type="primary"
            onClick={takeOrder.mutate}
            loading={takeOrder.isLoading}
            disabled={takeOrder.isSuccess}
          >
            {takeOrder.isSuccess ? '预定成功' : '预定'}
          </Button>,
        ]}
      >
        <div className="flex flex-row">
          ...
          <div className="w-1/2 rounded overflow-hidden">
            <Image
              ...
              placeholder={
                <div className="text-9xl text-center w-full">
                  <LoadingOutlined />
                </div>
              }
            />
...
const PackageComment = ({ package: p }) => {
  const commentService = useCommentService();
  commentService.getPackageComments(p.id);
};
